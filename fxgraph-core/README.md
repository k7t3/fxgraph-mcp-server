...
| クラス | 説明 |
|-------|------|
| `SceneGraphNode` | シーングラフの 1 ノードを表すデータクラス（ID・型名・子ノード・プロパティ等） |
...
```bash
./gradlew :fxgraph-core:build
```

このサブプロジェクト単体は JAR を生成しますが、通常は `fxgraph-cli` または `fxgraph-mcp-server` のビルド時に自動的に依存として組み込まれます。
