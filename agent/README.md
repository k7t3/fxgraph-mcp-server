...
| `agent.inspector` | `ChildrenGetter` | `Parent#getChildren()` を反射経由で安全に取得するユーティリティ |
| `agent.protocol` | `AgentCommand` / `AgentResponse` | エージェント独立の JSON 通信プロトコル定義 (core とは別コピー) |

> **注記:** `agent.protocol` は `core` サブプロジェクトのプロトコル定義と同一内容の独立コピーです。  
...
# 出力: agent/build/libs/fxgraph-agent.jar
```
